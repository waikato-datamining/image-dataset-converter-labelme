...
class LabelMeImageClassificationReader(Reader, VariableSupporter):
    ...
    def description(self) -> str:
        ...
        :rtype: str
        """
        return "Loads the image classification label from the associated labelme .json file."
    ...
    def _create_argparser(self) -> argparse.ArgumentParser:
        ...
        parser.add_argument("-i", "--input", type=str, help="Path to the json file(s) to read; glob syntax is supported; " + variable_list(obj=self), required=False, nargs="*")
        parser.add_argument("-I", "--input_list", type=str, help="Path to the text file(s) listing the json files to use; " + variable_list(obj=self), required=False, nargs="*")
    ...
    def read(self) -> Iterable:
        ...
        if "flags" in annotations:
            for l in annotations["flags"]:
                if isinstance(annotations["flags"][l], bool) and annotations["flags"][l]:
                    label = l
                    break
        ...
        yield ImageClassificationData(source=image, annotation=label)
